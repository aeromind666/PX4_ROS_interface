...
def main():
    ...
    while ((False == uav1_modes.state.armed) 
            or (False == uav2_modes.state.armed) 
            or (False == uav3_modes.state.armed)):
        uav1_modes.setArm()
        uav2_modes.setArm()
        uav3_modes.setArm()
        rate.sleep()
        # ic('set arm complete', uav1_modes.state.armed,uav2_modes.state.armed,uav3_modes.state.armed)
        # ic(rospy.Time.now())

    # uav1_modes.setTakeoff()
    # uav2_modes.setTakeoff()
    # uav3_modes.setTakeoff()
    # rate.sleep()
    # ic('set takeoff complete')

    k=0
    while k<10:
        sp1_pub.publish(cnt.sp)
        sp2_pub.publish(cnt.sp)
        sp3_pub.publish(cnt.sp)
        rate.sleep()
        k = k + 1
